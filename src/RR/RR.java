    ...
    public Process getNextProcess(){
        if (currentQueue != null) {
            ...
            if (!currentQueue.isEmpty()) {
                while (nextProcess == null || (globalClock.getTime() >= endTime && nextProcess.getExpectedRuntime() == nextProcess.getTimeLeft() && runningProcesses > 0)) { //ensure that after endtime only processes that have have run previously will run again
                    nextProcess = currentQueue.remove();
